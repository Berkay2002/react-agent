...
import { OPENAI_DEFAULT_MODEL_ENV_VARIABLE_NAME, Runner, } from "@openai/agents";
...
const TRACE_FLAG_ENV_KEYS = ["OPENAI_TRACE", "AGENT_TRACE_ENABLED"];
...
const selectFirstEnvValue = (keys) => {
...
const parseBooleanEnv = (keys, fallback) => {
...
const workflowName = selectFirstEnvValue(WORKFLOW_ENV_KEYS) ?? DEFAULT_WORKFLOW_NAME;
const modelOverride = selectFirstEnvValue(MODEL_ENV_KEYS);
const tracingEnabled = parseBooleanEnv(TRACE_FLAG_ENV_KEYS, false);
const includeSensitiveTraceData = parseBooleanEnv(TRACE_SENSITIVE_ENV_KEYS, false);
...
export async function runOnce(agent, input, context) {
