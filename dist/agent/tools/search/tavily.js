...
function isThrottled(tenantKey) {
    const now = Date.now();
...
function deriveRankScore(score, publishedDate, now) {
    ...
    const recencyScore = Math.max(0, 1 - Math.min(ageInDays, DAYS_IN_YEAR) / DAYS_IN_YEAR);
    return normalizedScore * SCORE_WEIGHT + recencyScore * RECENCY_WEIGHT;
}
export default tool({
    name: "tavily_search",
    description: "High-recall web search for facts. Returns url/title/snippet array.",
    parameters: z.object({
        query: z.string(),
        max_results: z
            .number()
